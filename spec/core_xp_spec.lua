...
describe("core/xp.lua", function()
    ...
    before_each(function()
        Ledger = {}
        -- MigrateDB (v2->v3, v3->v4) reescribe la serie xp y rellena los
        -- buckets de tiempo con los helpers genericos de core/series.lua
        -- y core/time_buckets.lua: hace falta cargarlos tambien, igual
        -- que en level_close_spec.lua/events_spec.lua.
    ...
    describe("version e InitDB", function()
        it("una base de datos nueva queda en la version actual", function()
            ...
            assert.are.equal(4, db.version)
            assert.are.equal(4, Ledger.DB_VERSION)
        end)
    ...
    describe("InitCharDB", function()
        ...
        it("tabla ya poblada: no altera nada de lo que ya hubiera", function()
            local original = {
                version  = 4,
                levels   = { [5] = { nivel = 5, totalXP = 100 } },
                sessions = { { t0 = 1000, nivel = 6 } },
            }

            local db = Ledger.InitCharDB(original)

            assert.are.equal(4, db.version)
            assert.are.same({ [5] = { nivel = 5, totalXP = 100 } }, db.levels)
            assert.are.same({ { t0 = 1000, nivel = 6 } }, db.sessions)
        end)
        ...
        it("tabla con version antigua: migra la version y conserva/crea el resto", function()
            ...
            -- de saber retroactivamente como se repartio ese tiempo).
            assert.are.same(
                { { t0 = 1, nivel = 3, buckets = { active = 0, idle = 0, travel = 0, dead = 0 } } },
                db.sessions)
        end)
    end)
    ...
    describe("migracion v2 -> v3: serie xp de stride 3 a 4", function()
        ...
        it("no repite la migracion si ya esta en la version actual", function()
            ...
            -- error, esto se corromperia.
            assert.are.same({ 0, 50, "kill", 5 }, session.e)
        end)
